...
class CheckResultRepository:
    ...
    async def get_stats_for_period(
        ...
        """Возвращает агрегированные статистики за период — для пересчёта DailyMetric."""
        stmt = select(
        ...
        return {
            ...
            "min_response_time": float(row.min_response_time) if row.min_response_time is not None else None,
